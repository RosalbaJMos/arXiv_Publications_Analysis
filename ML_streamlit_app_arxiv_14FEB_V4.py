...
st.subheader('This is an App that analyzes the abstracts of the last articles published on arXiv in 2021')
...
# 6. My function to clean and tokenize my text 
...
# Function to plot the number of articles per month
def articles(Publications2021):

    #Pubs_PhyMatCSBIO2021 = pd.read_csv('Pubs_PhyMatCSBIO2021.csv') 
    ...
    plt.ylabel('Number of articles published', size=16)
...
def main():
    ...
    st.markdown('***1.0 This plot shows the number of articles published per month on arXiv in 2021 in the fields of Physics, Maths, Computer Science, and Biology***')
    ...
    try:
        ...
        st.markdown('***2.0. This is the dataframe of ' + data_key + '. If you wish to analyze the topics of another field, please select the field on the sidebar at the left***')
        st.write(df.head(10))
        ...
        st.markdown('***2.2. These are the keywords that appear when dividing the field of ' + data_key +  ' into six main topics ***')
...
def my_predicting_machine(Abstract):
    ...
    st.write('Your abstract belongs to the field of :', Field_Predicted)

st.markdown('***3.0. In this part of the App, you can enter your abstract to gain insight on its corresponding category (Field)  ***')
